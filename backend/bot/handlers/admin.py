    ...
    if payload == "users":
        ...
        async with async_session_factory() as db:
            ...
            if not target_user:
                ...
                await db.commit()
                # ⚠️ FIX: this interpolated the raw telegram_username straight
                # into Markdown-parsed text. _md_escape() was already applied
                # to the "users" LIST view above (see its docstring) but was
                # missed here — so activating/banning any user whose username
                # contains an underscore (extremely common, e.g. "john_doe")
                # raised a Markdown parse BadRequest on every single click.
                # safe_edit_message() retries once as plain text so the
                # button didn't look fully "dead", but that's a wasted extra
                # API round-trip (added latency) on every such click, and the
                # bold formatting silently disappears. Escaping properly here
                # fixes it in one call, every time.
                uname = _md_escape(target_user.telegram_username or str(target_user.telegram_id))
                text = (
                    f"✅ {uname} activated."
                    if action == "activate"
                    else f"⛔ {uname} banned."
        ...
        if pending:
            for u in pending:
                # Same _md_escape gap as the activate/ban text above.
                text += f"👤 *{_md_escape(u.telegram_username or u.full_name or 'N/A')}* (`{u.telegram_id}`)\n"
        ...
        async with async_session_factory() as db:
            ...
            if target_user:
                if action == "approve":
                    ...
                    msg = f"✅ Approved {_md_escape(target_user.telegram_username or str(target_user.telegram_id))}"
                else:
                    target_user.status = UserStatus.INACTIVE
                    msg = f"❌ Rejected {_md_escape(target_user.telegram_username or str(target_user.telegram_id))}"
                await db.commit()
                text = msg
        ...
        if notifs:
            for n in notifs:
                ...
                # ⚠️ FIX: notification title/message is often auto-generated
                # text (exception messages, tracebacks, order IDs) that
                # routinely contains underscores — e.g. the payment-webhook
                # failure alerts this session added. Unescaped, that breaks
                # this whole "Notifications" button the same way unescaped
                # usernames broke "Manage Users".
                text += f"{status_emoji} *{_md_escape(n.title)}*: {_md_escape(n.message[:300])} ({n.severity})\n"
        else:
        ...
        text += f"ML Models Deployed: *SmAttaker (unified — crypto + gold + commodity + forex + stocks + futures)*\n"
