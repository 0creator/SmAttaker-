...
if TYPE_CHECKING:
    ...
    from backend.models.user_engagement import UserEngagement
...
class User(BaseModel):
    ...
    # ── RBAC tier (only meaningful when role == UserRole.ADMIN) ──────
    # See backend/utils/permissions.py for the AdminRole enum and what
    # each tier can do. NULL = legacy/unset, treated as full super-admin
    # access by role_has_permission() — see that module's docstring.
    admin_role: Mapped[Optional[str]] = mapped_column(String(32), nullable=True)
    ...
    # ── Trading Profile ──────────────────────────────────
    # Account type chosen at onboarding (or changed later in settings):
    ...
    default_account_type: Mapped[str] = mapped_column(
        String(16), default="paper", nullable=False
    )

    # ── Paper Trading Virtual Balance ──────────────────────
    # Tracks the user's virtual capital for PAPER mode. Starts at $10,000
    # (PAPER_INITIAL_BALANCE in signal_monitor.py). Updated by the signal
    # monitor when paper trades close (profit added / loss subtracted).
    paper_balance: Mapped[float] = mapped_column(
        "paper_balance",  # explicit column name for clarity in migrations
        nullable=False,
        default=10000.0,
    )
    paper_initial_balance: Mapped[float] = mapped_column(
    ...
    # ── Onboarding flag ────────────────────────────────────
    # True once the user has chosen their trading type (paper/demo/real).
    # New users must choose before they can use the bot's trading features.
    ...
    engagement: Mapped[Optional["UserEngagement"]] = relationship(
        "UserEngagement", back_populates="user", uselist=False, lazy="selectin"
    )
