...
class Signal(BaseModel):
    ...
    # ── Composite index for the dedup queries in strategies/runner.py ──
    # Every strategy cycle runs two lookups per symbol per cycle:
    #   WHERE symbol=... AND direction=... AND status='active'
    #   WHERE symbol=... AND direction=... AND created_at >= cutoff
    # With only single-column indexes on symbol/direction/status, Postgres
    # can use at most one of them and has to filter the rest row-by-row.
    # As the signals table grows (every ~15 min, indefinitely), that scan
    # gets slower every month — a real, gradual contributor to "signals
    # feel late," since a slow dedup check delays every single symbol in
    # the sequential analyze() loop, not just one.
    __table_args__ = (
        Index("ix_signals_symbol_direction_created", "symbol", "direction", "created_at"),
        Index("ix_signals_symbol_direction_status", "symbol", "direction", "status"),
    )
