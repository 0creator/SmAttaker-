...
MODEL_VERSION: str = 'v45.4.1'

# ═══════════════════════════════════════════════════════════════════
# v45.4 APEX CONFIG — DUAL-FILTER ENTRY GATE for WR BOOST
# ═══════════════════════════════════════════════════════════════════
TP_RR: float = 2.0
SL_ATR: float = 2.0
MAX_BARS: int = 8
...
# ── v45.4: ML base mode ──
APEX_ML_MODE: str = 'smart_filter_opt'

# ── v45.4 NEW: DUAL-FILTER ENTRY GATE (the key WR booster) ──
# After ML filter + Kelly sizing, apply a SECOND filter:
#   require ml_prob >= APEX_ML_PROB_FLOOR AND signal_quality >= APEX_SQ_FLOOR
# This drops "marginal" trades (low ML confidence OR low confluence)
# and keeps only HIGH-CONVICTION setups → WR jumps significantly.
# v45.4.1: TUNED floors to balance WR boost with freq retention.
#          Was 0.55/0.50 (too strict → freq collapsed to 0.025/day on BTC).
#          Now 0.45/0.30 (sweet spot: WR +10%+, freq ≥ 0.8/day).
APEX_DUAL_FILTER: bool = True
APEX_ML_PROB_FLOOR: float = 0.45        # v45.4.1: 0.55 → 0.45 (relaxed to keep freq)
APEX_SQ_FLOOR: float = 0.30             # v45.4.1: 0.50 → 0.30 (relaxed to keep freq)

# ── v45.3 retained: Regime-Adaptive Sizing ──
APEX_REGIME_SIZING: bool = True
APEX_KELLY_FRACTION: float = 0.40
APEX_SIZE_FLOOR: float = 0.30
APEX_SIZE_CAP: float = 1.80
APEX_BOOST_TREND: float = 1.15
APEX_CUT_CHOP: float = 0.60
APEX_CUT_HIGHVOL: float = 0.70

# ── v45.3 retained: Drawdown Brake (size only, no halt) ──
APEX_DRAWDOWN_BRAKE: bool = True
APEX_DD_SOFT_LIMIT: float = 0.10
APEX_DD_HARD_LIMIT: float = 0.18
APEX_DD_HALT_LIMIT: float = 0.99         # disabled

# ── v45.3 retained: Dynamic TP/SL ──
APEX_DYNAMIC_EXIT: bool = True
APEX_TP_RR_TREND: float = 3.0
APEX_TP_RR_RANGE: float = 1.8
APEX_MAX_BARS_TREND: int = 14
APEX_MAX_BARS_RANGE: int = 6

# ── v45.3 retained: Signal Quality ──
APEX_SIGNAL_QUALITY: bool = True
APEX_QUALITY_THRESHOLD: float = 0.40     # pre-filter (before dual gate)
APEX_QUALITY_BOOST_THRESHOLD: float = 0.70

# ── v45.5 NEW: Low-Vol as BONUS, not GATE ──
# DIAGNOSIS: 'lv' (ATR% below its 168-bar median) was a hard AND-filter on
# 7 of the 10 original v43 triggers (IDs 0,1,2,3,4,5,8). During a strong
# trending week (BTC/Gold breakout — exactly when ATR expands), 'lv' is
# always False, so those triggers can NEVER fire — the system goes silent
# right when the best opportunities show up. This mirrors the same
# freq-collapse problem the v45.4.1 dual-filter floors ran into.
# FIX: 'lv' no longer gates trigger firing (removed from build_triggers).
# It now only gives a small position-size boost when present, same
# pattern as the existing regime boost/cut multipliers below.
APEX_LV_BONUS: bool = True
APEX_LV_BONUS_MULT: float = 1.10

# ── v45.3 retained: Anti-Overtrading ──
APEX_MAX_TRADES_PER_DIR: int = 3
APEX_MIN_BARS_BETWEEN_ENTRIES: int = 0

# ── v45.2 retained: Volatility Targeting ──
APEX_VOL_TARGETING: bool = True
APEX_TARGET_ANNUAL_VOL: float = 0.45
APEX_VOL_LOOKBACK: int = 50
APEX_VOL_FLOOR: float = 0.50
APEX_VOL_CAP: float = 1.20

# ── v45.2 retained: Per-Trade Risk Cap ──
APEX_MAX_RISK_PER_TRADE: float = 0.02
...
def build_features(df: pd.DataFrame) -> pd.DataFrame:
    """Build the complete v45 feature set on a raw OHLCV DataFrame.

    SINGLE SOURCE OF TRUTH for feature engineering. Used by BOTH:
      - Offline training path: load_and_build_features() → this function
      - Live inference path:   V45Strategy._build_features() → this function

    v45.4.2 FIX (KeyError 'bosL_5ago' in production): the live strategy
    previously carried an inlined COPY of this logic that had drifted to
    the v43 feature subset (stopped at bbU). build_triggers() then crashed
    on the first v44+ column it touched (bosL_5ago, trigger ID 10), and
    the meta-model silently received zeros for sw_lo_5 / sw_hi_5 /
    close_4h / imbalance / bosL_5ago features. Centralizing here makes
    train/live feature drift structurally impossible.

    Input:  DataFrame with columns [Open, High, Low, Close, Volume] and a
            tz-naive DatetimeIndex (required for session features).
    Output: same DataFrame with all v43 + v44 VES/RETR + v45 APEX features.
    """
    d = df.copy()
    ...
    # ═══════════════════════════════════════════════════════════════════
    # v44 EXTENSION — VES/RETR FEATURES (high-vol + retracement sniper)
    # These features power the 9 new triggers (IDs 10-18).
    # The original v43 triggers (IDs 0-9) are UNCHANGED.
    # ═══════════════════════════════════════════════════════════════════
    # High-volatility regime (opposite of lv)
    atr_pct_med = atr_pct.rolling(168, min_periods=50).median()
    d['hv']  = atr_pct > atr_pct_med
    d['hvx'] = atr_pct > atr_pct_med * 1.3

    # ADX (Average Directional Index) — Wilder's smoothing, causal
    # Used by VES/RETR triggers as a trend-strength filter
    _up_move = d['High'].diff()
    _down_move = -d['Low'].diff()
    _plus_dm = pd.Series(np.where((_up_move > _down_move) & (_up_move > 0), _up_move, 0.0), index=d.index)
    _minus_dm = pd.Series(np.where((_down_move > _up_move) & (_down_move > 0), _down_move, 0.0), index=d.index)
    _tr_for_adx = pd.concat([d['High'] - d['Low'],
                              (d['High'] - d['Close'].shift()).abs(),
                              (d['Low'] - d['Close'].shift()).abs()], axis=1).max(axis=1)
    _atr_w = _tr_for_adx.ewm(alpha=1/14, adjust=False).mean()
    _plus_di = _plus_dm.ewm(alpha=1/14, adjust=False).mean() / (_atr_w + 1e-9) * 100
    _minus_di = _minus_dm.ewm(alpha=1/14, adjust=False).mean() / (_atr_w + 1e-9) * 100
    _dx = (_plus_di - _minus_di).abs() / (_plus_di + _minus_di + 1e-9) * 100
    d['adx'] = _dx.ewm(alpha=1/14, adjust=False).mean()
    d['adx_s']  = d['adx'] > 25   # strong trend
    d['adx_vs'] = d['adx'] > 30   # very strong trend

    # Swing levels (prior 20-bar high/low, EXCLUDES current bar via shift(1))
    d['sw_lo_20'] = d['Low'].shift(1).rolling(20).min()
    d['sw_hi_20'] = d['High'].shift(1).rolling(20).max()
    d['sw_lo_5']  = d['Low'].shift(1).rolling(5).min()
    d['sw_hi_5']  = d['High'].shift(1).rolling(5).max()

    # Liquidity Sweep (LSL/LSS) — sniper reversal pattern
    d['lsl'] = (d['Low'] < d['sw_lo_20']) & (d['Close'] > d['sw_lo_20']) & \
               (d['Close'] > d['Open']) & (d['cp'] > 0.5)
    d['lss'] = (d['High'] > d['sw_hi_20']) & (d['Close'] < d['sw_hi_20']) & \
               (d['Close'] < d['Open']) & (d['cp'] < 0.5)

    # Break of Structure (BOS) — first close beyond prior swing
    d['bosL']  = d['Close'] > d['sw_hi_20']
    d['bosS']  = d['Close'] < d['sw_lo_20']
    d['bosL5'] = d['Close'] > d['sw_hi_5']
    d['bosS5'] = d['Close'] < d['sw_lo_5']

    # EMA21 Slope Flip — first bar of trend turn
    _e21_slope_1 = d['e21'].diff()
    d['slopeFlipL'] = (_e21_slope_1 > 0) & (_e21_slope_1.shift(1) <= 0)
    d['slopeFlipS'] = (_e21_slope_1 < 0) & (_e21_slope_1.shift(1) >= 0)

    # VWAP (rolling 24-bar) and deviation
    _typical = (d['High'] + d['Low'] + d['Close']) / 3
    _vol_sum_24 = d['Volume'].rolling(24).sum()
    d['vwap'] = (_typical * d['Volume']).rolling(24).sum() / (_vol_sum_24 + 1e-9)
    _vwap_std = (_typical - d['vwap']).rolling(24).std()
    d['vwap_dev'] = (d['Close'] - d['vwap']) / (_vwap_std + 1e-9)
    d['vwapL'] = (d['vwap_dev'] < -1.5) & (d['Close'] > d['Open'])
    d['vwapS'] = (d['vwap_dev'] > 1.5) & (d['Close'] < d['Open'])

    # Volume Climax — capitulation volume
    _vmean50 = d['Volume'].rolling(50).mean()
    d['vclimax'] = d['Volume'] > 2.5 * _vmean50
    d['vclxL'] = d['vclimax'] & (d['Close'] > d['Open']) & (d['Close'] > d['vwap'])
    d['vclxS'] = d['vclimax'] & (d['Close'] < d['Open']) & (d['Close'] < d['vwap'])

    # Microstructure HH+HL (2 consecutive) — earliest trend confirm
    d['microL'] = (d['Low'] > d['Low'].shift(1)) & (d['Low'].shift(1) > d['Low'].shift(2)) & \
                  (d['High'] > d['High'].shift(1))
    d['microS'] = (d['High'] < d['High'].shift(1)) & (d['High'].shift(1) < d['High'].shift(2)) & \
                  (d['Low'] < d['Low'].shift(1))

    # ATR Expansion Breakout — first bar with TR > 1.8×ATR
    d['tr_local'] = _tr_for_adx
    d['atrExpL'] = (_tr_for_adx > 1.8 * d['a']) & (d['Close'] > d['Open']) & \
                   (d['Close'] > d['High'].shift(1))
    d['atrExpS'] = (_tr_for_adx > 1.8 * d['a']) & (d['Close'] < d['Open']) & \
                   (d['Close'] < d['Low'].shift(1))

    # Pullback zones (for RETR triggers)
    d['dist_e50'] = (d['Close'] - d['e50']) / d['e50']
    d['pb_L']  = ((d['Low'] <= d['e21'] * 1.005) & (d['Close'] > d['e21'] * 0.998)).fillna(False)
    d['pb_S']  = ((d['High'] >= d['e21'] * 0.995) & (d['Close'] < d['e21'] * 1.002)).fillna(False)
    d['pbL50'] = ((d['Low'] <= d['e50'] * 1.010) & (d['Close'] > d['e50'] * 0.995)).fillna(False)
    d['pbS50'] = ((d['High'] >= d['e50'] * 0.990) & (d['Close'] < d['e50'] * 1.005)).fillna(False)

    # Reversal bar quality (sharp rejection)
    d['revB'] = (d['Close'] > d['Open']) & (d['cp'] > 0.55)
    d['revS'] = (d['Close'] < d['Open']) & (d['cp'] < 0.45)

    # ═══════════════════════════════════════════════════════════════════
    # v45 APEX ELITE FEATURES — for the 4 new elite triggers (IDs 22-25)
    # ═══════════════════════════════════════════════════════════════════

    # ── Order Flow Imbalance Proxy (volume × body direction) ──
    # buy_pressure = volume when close > open (bullish conviction)
    # sell_pressure = volume when close < open (bearish conviction)
    # imbalance = (buy - sell) / (buy + sell), range [-1, +1]
    bar_signed = np.sign(d['Close'] - d['Open'])
    d['imbalance_raw'] = d['Volume'] * bar_signed
    imb_roll = d['imbalance_raw'].rolling(10).sum()
    vol_roll = d['Volume'].rolling(10).sum()
    d['imbalance'] = (imb_roll / (vol_roll + 1e-9)).fillna(0)
    d['imbB'] = d['imbalance'] > 0.25   # strong buy imbalance
    d['imbS'] = d['imbalance'] < -0.25  # strong sell imbalance

    # ── Exhaustion Reversal (sharp move + extreme RSI + reversal candle) ──
    # v45: Originally used "gap" but crypto H1 has very few gaps (24/7 market).
    # Redesigned: detects sharp 3-bar move + RSI extreme + reversal candle.
    # This catches panic-selling/buying climaxes → sharp reversals.
    ret_3bar = (d['Close'] - d['Close'].shift(3)) / (d['Close'].shift(3) + 1e-9)
    d['ret_3bar'] = ret_3bar
    # Bullish exhaustion: sharp 3-bar DROP (>2%) + RSI<35 + reversal bar (green, strong close)
    d['exhB'] = (ret_3bar < -0.02) & (d['rsi'] < 35) & (d['Close'] > d['Open']) & (d['cp'] > 0.55)
    # Bearish exhaustion: sharp 3-bar RISE (>2%) + RSI>65 + reversal bar (red, weak close)
    d['exhS'] = (ret_3bar > 0.02) & (d['rsi'] > 65) & (d['Close'] < d['Open']) & (d['cp'] < 0.45)

    # ── Multi-Timeframe Confirmation (4h trend via 4-bar EMA on H1) ──
    # 4h close ≈ every 4 H1 bars; resample by rolling mean
    d['close_4h'] = d['Close'].rolling(4).mean()
    d['e21_4h'] = d['close_4h'].ewm(span=21, adjust=False).mean()
    d['e50_4h'] = d['close_4h'].ewm(span=50, adjust=False).mean()
    d['mtfBull'] = (d['close_4h'] > d['e21_4h']) & (d['e21_4h'] > d['e50_4h'])
    d['mtfBear'] = (d['close_4h'] < d['e21_4h']) & (d['e21_4h'] < d['e50_4h'])

    # ── Session-Aware (Asia/Europe/US) ──
    hours = d.index.hour
    d['asia_session'] = ((hours >= 0) & (hours < 8)).astype(int)
    d['eu_session']   = ((hours >= 8) & (hours < 16)).astype(int)
    d['us_session']   = ((hours >= 16) & (hours < 24)).astype(int)
    # US session + London overlap (high liquidity): 13-16 UTC
    d['us_eu_overlap'] = ((hours >= 13) & (hours < 16)).astype(int)

    # ── Signal Quality Composite (0-1) ──
    # Confluence of 5 leading factors: trend alignment, ADX strength, vol surge,
    # imbalance alignment, MTF confirmation
    sq = 0.0
    # 1. Trend alignment (0 or 0.20)
    sq = sq + 0.20 * ((d['Close'] > d['e21']) & (d['e21'] > d['e50'])).astype(float)
    # 2. ADX strength (0 or 0.20)
    sq = sq + 0.20 * (d['adx'] > 25).astype(float)
    # 3. Volume surge (0 or 0.20)
    sq = sq + 0.20 * (d['vr'] > 1.5).astype(float)
    # 4. Buy imbalance aligned with green bar (0 or 0.20)
    sq = sq + 0.20 * (d['imbB'] & (d['Close'] > d['Open'])).astype(float)
    # 5. MTF confirmation (0 or 0.20)
    sq = sq + 0.20 * d['mtfBull'].astype(float)
    d['signal_quality'] = sq

    # BOS within last 5 bars (for breakout-retest pattern)
    # PERF: use rolling(5).max() instead of rolling(5).apply(any) — ~10x faster
    d['bosL_5ago'] = d['bosL'].rolling(5).max().fillna(0).astype(bool)
    d['bosS_5ago'] = d['bosS'].rolling(5).max().fillna(0).astype(bool)
...
def load_and_build_features() -> pd.DataFrame:
    ...
    v45.4.2: feature engineering now delegated to build_features() — the same
    single source of truth the live inference path uses (see V45Strategy).
    """
    d = _load_raw_ohlcv()
    return build_features(d)

# ═══════════════════════════════════════════════════════════════════
# STRATEGY LOGIC — v44.1: 22 TRIGGERS (10 v43 + 12 VES/RETR, balanced LONG/SHORT)
# ═══════════════════════════════════════════════════════════════════
def build_triggers(df: pd.DataFrame) -> list:
    """v44.1: 10 v43 original triggers (IDs 0-9) + 12 VES/RETR triggers (IDs 10-21).

    v44.1 FIXES (based on diagnostic of dead/sparse triggers on real BTC+ETH data):
      - ID 9  (S4_OB_VH_CQ2_VolSurge): was DEAD (0 fires) — relaxed `vs`→`vh`
      - ID 11 (VES_L2_LiquiditySweep): was SPARSE (4 fires) — dropped `hv`, relaxed `vs`→`(vs|vh)`, `rB`→`rBe`
      - ID 12 (VES_L3_VWAP_Reclaim):   was DEAD (0 fires) — dropped `hv` and `rB`, kept `rBe`+vol
      - ID 13 (VES_L4_SlopeFlip_ADX):  was SPARSE (10) — relaxed `vs`→`(vs|vh)`
      - ID 17 (VES_S2_LiquiditySweepHigh): was SPARSE (4) — same fixes as ID 11
      - ADDED 3 new SHORT triggers for symmetry:
        * ID 19 (VES_S3_VWAP_Reject):   SHORT mirror of VES_L3
        * ID 20 (VES_S4_SlopeFlip_ADX): SHORT mirror of VES_L4
        * ID 21 (RETR_S2_ShallowPullback): SHORT mirror of RETR_L2

    All 12 new triggers use LEADING indicators (liquidity sweeps, BOS, slope flips,
    VWAP reclaims) and fire during HIGH-volatility regimes (when v43 is blind).
    All signals use shift(1) — zero lookahead.
    """
    sb = lambda c: df[c].shift(1).fillna(False).astype(bool)
    NO = pd.Series(False, df.index)

    # Pre-compute trend-alignment helpers (causal via shift(1))
    above21 = (df['Close'].shift(1) > df['e21'].shift(1)).fillna(False).astype(bool)
    below21 = (df['Close'].shift(1) < df['e21'].shift(1)).fillna(False).astype(bool)

    # ── ORIGINAL v43 TRIGGERS (IDs 0-9) ──
    # IDs 0-8 byte-for-byte identical to v35/v36/v37/v43.
    # ID 9 FIXED: was `rsiVH & c2S & vs & rSe` (DEAD — 0 fires on 50k bars)
    #             → relaxed `vs`→`vh` (vol ratio > 1.3 instead of > 2.0)
    # v45.5: 'lv' REMOVED as a hard AND-gate from all 7 triggers below (was
    # killing signal freq during high-vol/strong-trend weeks — see
    # APEX_LV_BONUS in config). It now only contributes a size bonus in
    # _compute_apex_boost(), never blocks a trigger from firing.
    triggers = [
        (sb('rB') & sb('sB') & sb('tB') & sb('c2B') & sb('vs') & sb('m3B') & sb('e21u'), NO),
        (sb('sB') & sb('tB') & sb('near21') & sb('c2B') & sb('riB') & sb('rB'), NO),
        (sb('rBe') & sb('sB') & sb('tB') & sb('c2B') & sb('vh') & sb('m2B') & sb('e21u') & sb('stackB'), NO),
        (sb('rB') & sb('sB') & sb('tB') & sb('c3B') & sb('vv') & sb('m3B') & sb('e21u'), NO),
        (sb('rB') & sb('sB') & sb('tB') & sb('c2B') & sb('vh') & sb('m2B') & sb('e21u'), NO),
        (sb('rBe') & sb('sB') & sb('tB') & sb('c2B') & sb('vh') & sb('m2B') & sb('e21u'), NO),
        (NO, sb('rsiH') & sb('c3S') & sb('vs') & sb('rSe')),
        (NO, sb('rsiH') & sb('bbU') & sb('c2S') & sb('vs')),
        (NO, sb('rS') & sb('sS') & sb('tS') & sb('c2S') & sb('vs') & sb('m3S') & sb('e21d')),
        # ID 9 FIXED (was DEAD): rsiVH & c2S & vh & rSe  (relaxed vs→vh)
        (NO, sb('rsiVH') & sb('c2S') & sb('vh') & sb('rSe')),

        # ── v44.1 VES-L (Volatility Expansion Sniper, LONG) IDs 10-13 ──
        # VES-L1 (ID 10): Breakout + Retest + Reversal (high vol) — HEALTHY (109)
        (sb('bosL_5ago') & sb('rB') & sb('tB') & sb('pb_L') & sb('revB') & (sb('vs') | sb('vh')) & sb('sB') & sb('hv'), NO),
        # VES-L2 (ID 11) FIXED v44.1: was SPARSE (20) — dropped `above21` (conflicts with lsl semantics)
        # lsl = sweep low (price dipped below swing then recovered) — by definition price is near e21 or below,
        # so requiring above21 kills it. rBe alone is sufficient trend filter.
        (sb('lsl') & sb('rBe') & (sb('vs') | sb('vh')), NO),
        # VES-L3 (ID 12) FIXED v44.1: was DEAD (0) — dropped `above21` (same reason as ID 11)
        # vwapL = dev<-1.5σ (price below VWAP) — above21 rarely co-occurs.
        (sb('vwapL') & sb('rBe') & (sb('vs') | sb('vh')), NO),
        # VES-L4 (ID 13) FIXED: was SPARSE (10) — relaxed `vs`→`(vs|vh)` → now HEALTHY (47)
        (sb('slopeFlipL') & sb('adx_s') & (sb('vs') | sb('vh')) & sb('rBe') & sb('hv'), NO),

        # ── v44.1 RETR-L (Retracement Sniper, LONG) IDs 14-15 ──
        # RETR-L1 (ID 14): Deep Pullback to EMA50 + Reversal — HEALTHY (345)
        (sb('rB') & sb('tB') & sb('sB') & sb('adx_s') & sb('pbL50') & sb('revB') &
         (df['rsi'].shift(1) > 35) & (df['rsi'].shift(1) < 60) & sb('e21u'), NO),
        # RETR-L2 (ID 15): Shallow Pullback to EMA21 + Reversal — HEALTHY (73)
        (sb('rB') & sb('tB') & sb('sB') & sb('adx_s') & sb('pb_L') & sb('revB') &
         (df['rsi'].shift(1) > 40) & (df['rsi'].shift(1) < 65) & sb('e21u') & sb('vh'), NO),

        # ── v44.1 VES-S / RETR-S (SHORT, asymmetric) IDs 16-18 ──
        # VES-S1 (ID 16): Breakdown + Retest + Reversal (high vol) — HEALTHY (109)
        (NO, sb('bosS_5ago') & sb('rS') & sb('tS') & sb('pb_S') & sb('revS') & (sb('vs') | sb('vh')) & sb('sS') & sb('hv')),
        # VES-S2 (ID 17) FIXED v44.1: was SPARSE (21) — dropped `below21` (mirror of ID 11 fix)
        (NO, sb('lss') & sb('rSe') & (sb('vs') | sb('vh'))),
        # RETR-S1 (ID 18): Deep Pullback to EMA50 + Reversal (SHORT) — HEALTHY (408)
        (NO, sb('rS') & sb('tS') & sb('sS') & sb('adx_s') & sb('pbS50') & sb('revS') &
         (df['rsi'].shift(1) < 65) & (df['rsi'].shift(1) > 40) & sb('e21d')),

        # ── v44.1 NEW: 3 SHORT triggers for symmetry (IDs 19-21) ──
        # VES-S3 (ID 19) FIXED v44.1: was DEAD (0) — dropped `below21` (mirror of ID 12 fix)
        # vwapS = dev>+1.5σ (price above VWAP) — below21 rarely co-occurs.
        (NO, sb('vwapS') & sb('rSe') & (sb('vs') | sb('vh'))),
        # VES-S4 (ID 20): Slope Flip + ADX (SHORT mirror of VES_L4 ID 13) — HEALTHY (68)
        (NO, sb('slopeFlipS') & sb('adx_s') & (sb('vs') | sb('vh')) & sb('rSe') & sb('hv')),
        # RETR-S2 (ID 21): Shallow Pullback to EMA21 (SHORT mirror of RETR_L2 ID 15) — HEALTHY (106)
        (NO, sb('rS') & sb('tS') & sb('sS') & sb('adx_s') & sb('pb_S') & sb('revS') &
         (df['rsi'].shift(1) < 60) & (df['rsi'].shift(1) > 35) & sb('e21d') & sb('vh')),

        # ═══════════════════════════════════════════════════════════════════
        # v45 APEX ELITE TRIGGERS (IDs 22-25) — Confluence-based, highest quality
        # ═══════════════════════════════════════════════════════════════════

        # ── APEX-L1 (ID 22): Order Flow Imbalance + Trend Confluence ──
        # Strong buy imbalance + MTF bull + reversal bar + regime bull
        # Catches institutional accumulation with multi-timeframe confirmation.
        (sb('imbB') & sb('mtfBull') & sb('rB') & sb('revB') & (sb('vs') | sb('vh')), NO),

        # ── APEX-L2 (ID 23): Exhaustion Gap Reversal + High Quality ──
        # Bullish exhaustion gap (gap down + RSI<30 + reversal bar)
        # Catches panic-selling climaxes → sharp reversals.
        # Signal quality composite ≥ 0.40 ensures confluence.
        (sb('exhB') & (df['signal_quality'].shift(1) >= 0.40), NO),

        # ── APEX-S1 (ID 24): Order Flow Imbalance + Trend Confluence (SHORT) ──
        # Mirror of ID 22 for SHORT.
        (NO, sb('imbS') & sb('mtfBear') & sb('rS') & sb('revS') & (sb('vs') | sb('vh'))),

        # ── APEX-S2 (ID 25): Exhaustion Gap Reversal (SHORT) + High Quality ──
        # Bearish exhaustion gap (gap up + RSI>70 + reversal bar)
        # Catches panic-buying climaxes → sharp reversals.
        (NO, sb('exhS') & (df['signal_quality'].shift(1) >= 0.40)),
    ]
    return triggers

N_TRIGGERS = 26

# ── v44.1: Trigger ID → human-readable name ──
TRIGGER_NAMES = {
    # Original v43 (low-vol sniper)
    0:  "L1_Regime_CQ2_VolSurge_Mom3",
    1:  "L2_Pullback_EMA21",
    2:  "L3_Relax_Stack",
    3:  "L4_Regime_CQ3_VolVHi",
    4:  "L5_Regime_CQ2_VolHi_M2",
    5:  "L6_E200_CQ2_VolHi_M2",
    6:  "S1_OB_CQ3_VolSurge",
    7:  "S2_OB_Bollinger_CQ2_VolSurge",
    8:  "S3_Sym_FullBear",
    9:  "S4_OB_VH_CQ2_VolHi",          # FIXED (was VolSurge)
    # VES-L: Volatility Expansion Sniper (LONG, high-vol)
    10: "VES_L1_Breakout_Retest_Reversal",
    11: "VES_L2_LiquiditySweep",        # FIXED (dropped hv, rB→rBe)
    12: "VES_L3_VWAP_Reclaim",          # FIXED (dropped hv, rB→rBe)
    13: "VES_L4_SlopeFlip_ADX",         # FIXED (vs→vs|vh)
    # RETR-L: Retracement Sniper (LONG)
    14: "RETR_L1_DeepPullback_EMA50",
    15: "RETR_L2_ShallowPullback_EMA21",
    # VES-S / RETR-S (SHORT)
    16: "VES_S1_Breakdown_Retest_Reversal",
    17: "VES_S2_LiquiditySweepHigh",    # FIXED
    18: "RETR_S1_DeepPullback_EMA50",
    # v44.1 NEW: 3 SHORT triggers for symmetry
    19: "VES_S3_VWAP_Reject",           # NEW (mirror of VES_L3)
    20: "VES_S4_SlopeFlip_ADX",         # NEW (mirror of VES_L4)
    21: "RETR_S2_ShallowPullback_EMA21",# NEW (mirror of RETR_L2)
    # v45 APEX ELITE: Confluence-based, highest quality (IDs 22-25)
    22: "APEX_L1_Imbalance_MTF_Confluence",  # Order flow + multi-timeframe
    23: "APEX_L2_ExhaustionGap_Reversal",    # Panic climax reversal
    24: "APEX_S1_Imbalance_MTF_Confluence",  # SHORT mirror of APEX_L1
    25: "APEX_S2_ExhaustionGap_Reversal",    # SHORT mirror of APEX_L2
}
...
# ═══════════════════════════════════════════════════════════════════
# v45 APEX BACKTEST ENGINE — Regime-Adaptive + Drawdown Brake + Dynamic Exit
# ═══════════════════════════════════════════════════════════════════
def _compute_apex_size_mult(df: pd.DataFrame, i: int, direction: int,
                              equity: float, peak_equity: float,
                              base_p: float = 0.5) -> float:
    """v45 APEX: Compute regime-adaptive position size multiplier.

    Combines:
      1. Kelly fraction from ML probability (passed as base_p)
      2. Regime boost (trending+aligned) or cut (choppy/high-vol)
      3. Drawdown brake (soft/hard/halt)
      4. Signal quality boost
    """
    if not APEX_REGIME_SIZING:
        return 1.0

    mult = 1.0

    # ── 1. Kelly base (half-Kelly on probability edge) ──
    b = TP_RR
    p = base_p
    f_p = (p * b - (1 - p)) / b
    f_base = (0.5 * b - 0.5) / b  # baseline at p=0.5
    if f_base > 0 and f_p > 0:
        ratio = f_p / f_base
        kelly_mult = 1.0 + (ratio - 1.0) * APEX_KELLY_FRACTION * 4.0
        mult *= kelly_mult

    # ── 2. Regime adjustment (using prior bar data — leak-free) ──
    fb = i - 1
    if fb >= 0 and 'adx' in df.columns:
        adx_val = df['adx'].values[fb] if not np.isnan(df['adx'].values[fb]) else 20.0
        atrp = df['atrp'].values[fb] if 'atrp' in df.columns and not np.isnan(df['atrp'].values[fb]) else 0.0
        atrp_med = df['atrp'].rolling(168, min_periods=50).median().values[fb] if 'atrp' in df.columns else atrp
        atrp_med = atrp_med if not np.isnan(atrp_med) else atrp

        # Trend boost: ADX>30 + aligned with trade direction
        if adx_val > 30:
            # Check alignment: trendB for LONG, trendS for SHORT
            if direction == 1 and 'tB' in df.columns and bool(df['tB'].values[fb]):
                mult *= APEX_BOOST_TREND
            elif direction == -1 and 'tS' in df.columns and bool(df['tS'].values[fb]):
                mult *= APEX_BOOST_TREND
        # Chop cut: ADX<20
        elif adx_val < 20:
            mult *= APEX_CUT_CHOP

        # High-vol cut: ATR% > 2× median
        if atrp_med > 0 and atrp > 2.0 * atrp_med:
            mult *= APEX_CUT_HIGHVOL

    # ── 3. Drawdown brake ──
    if APEX_DRAWDOWN_BRAKE and peak_equity > 0:
        dd = (peak_equity - equity) / peak_equity
        if dd >= APEX_DD_HALT_LIMIT:
            return 0.0  # halt new entries
        elif dd >= APEX_DD_HARD_LIMIT:
            mult *= 0.40
        elif dd >= APEX_DD_SOFT_LIMIT:
            mult *= 0.70

    # ── 4. Signal quality boost ──
    if APEX_SIGNAL_QUALITY and 'signal_quality' in df.columns and fb >= 0:
        sq = df['signal_quality'].values[fb]
        if not np.isnan(sq) and sq >= APEX_QUALITY_BOOST_THRESHOLD:
            mult *= 1.20  # 20% boost for high-quality signals

    return float(np.clip(mult, APEX_SIZE_FLOOR, APEX_SIZE_CAP))


def _compute_apex_exit_params(df: pd.DataFrame, i: int, direction: int) -> tuple:
    """v45 APEX: Compute dynamic TP_RR and MAX_BARS based on ADX regime.

    Returns (tp_rr, max_bars) — wider TP and longer hold in strong trends.
    """
    if not APEX_DYNAMIC_EXIT:
        return TP_RR, MAX_BARS

    fb = i - 1
    if fb < 0 or 'adx' not in df.columns:
        return TP_RR, MAX_BARS

    adx_val = df['adx'].values[fb]
    if np.isnan(adx_val):
        return TP_RR, MAX_BARS

    if adx_val > 30:
        # Strong trend → wider TP, longer hold
        return APEX_TP_RR_TREND, APEX_MAX_BARS_TREND
    elif adx_val < 20:
        # Chop → tighter TP, shorter hold
        return APEX_TP_RR_RANGE, APEX_MAX_BARS_RANGE
    else:
        return TP_RR, MAX_BARS


def backtest_apex(df: pd.DataFrame, trades_input: list,
                   meta: dict = None, apply_ml: bool = True,
                   max_positions: int = MAX_POSITIONS,
                   capital: float = INITIAL_CAPITAL) -> tuple:
    """v45 APEX v2: smart_filter_opt Kelly sizing as BASE + APEX enhancements as BOOST/CUT.

    CRITICAL FIX (v45.1): The previous backtest_apex was REGRESSIVE because it
    replaced the ML Kelly sizing with a separate size_mult calculation that
    didn't leverage the ML probability correctly. This version:

      1. Uses apply_true_meta_to_trades(mode='smart_filter_opt') to get the
         EXACT same Kelly-sized trades that made v44.1+ML perform so well
      2. Then LAYERS the APEX enhancements on TOP of that base sizing:
         - Regime boost (trending+aligned → +30%)
         - Chop cut (ADX<20 → -40%)
         - Drawdown brake (soft/hard/halt, but applied to risk_amount
           NOT as a trade filter — ML still decides entries)
         - Signal quality boost (+20% for high-confluence signals)
      3. Dynamic exit (TP_RR / MAX_BARS scaled by ADX) is still applied

    This preserves the v44.1+ML performance while ADDING the APEX upside.
    """
    cl = df['Close'].values
    hi = df['High'].values
    lo = df['Low'].values
    ...
    # ── STEP 1: Get ML-filtered + Kelly-sized trades ──
    # v45.3: Use APEX_ML_MODE (strict_filter_opt for higher WR)
    if apply_ml and meta is not None:
        feat_matrix = build_meta_feature_matrix(df)
        ml_trades, _ = apply_true_meta_to_trades(
            trades_input, feat_matrix, meta, mode=APEX_ML_MODE
        )
    else:
        ml_trades = list(trades_input)

    # ── STEP 1.5: v45.3 Signal Quality PRE-FILTER ──
    # Drop trades where the entry bar's signal_quality < APEX_QUALITY_THRESHOLD
    # This boosts WR by ensuring only high-confluence setups are traded.
    if APEX_SIGNAL_QUALITY and 'signal_quality' in df.columns:
        sq_vals = df['signal_quality'].values
        filtered = []
        for t in ml_trades:
            fb = t['ebar'] - 1
            if fb >= 0 and fb < len(sq_vals):
                sq = sq_vals[fb]
                if not np.isnan(sq) and sq >= APEX_QUALITY_THRESHOLD:
                    filtered.append(t)
            else:
                filtered.append(t)  # keep if can't evaluate
        ml_trades = filtered

    # ── STEP 1.6: v45.4 NEW — DUAL-FILTER ENTRY GATE ──
    # The KEY WR booster: drop trades where ML prob < 0.55 OR sq < 0.50.
    # This is applied AFTER ML filter+Kelly sizing (which already dropped
    # low-prob trades via threshold), so it's a SECOND, stricter gate.
    # Result: only HIGH-CONVICTION trades survive → WR jumps to 55%+.
    if APEX_DUAL_FILTER:
        sq_vals = df['signal_quality'].values if 'signal_quality' in df.columns else None
        filtered = []
        for t in ml_trades:
            # ML prob floor
            ml_p = t.get('meta_prob', 0.5)
            if ml_p < APEX_ML_PROB_FLOOR:
                continue
            # Signal quality floor
            if sq_vals is not None:
                fb = t['ebar'] - 1
                if fb >= 0 and fb < len(sq_vals):
                    sq = sq_vals[fb]
                    if np.isnan(sq) or sq < APEX_SQ_FLOOR:
                        continue
            filtered.append(t)
        ml_trades = filtered

    # ── STEP 2: Index ML trades by entry bar ──
    trades_by_bar = {}
    for t in ml_trades:
        trades_by_bar.setdefault(t['ebar'], []).append(t)
    ...
    for i in range(n):
        # ── EXIT CHECK (with dynamic timeout) ──
        survived = []
        for pos in positions:
            sd = pos['dir']
            exit_price = None
            ...
            if exit_type is None and i - pos['ebar'] >= pos['max_bars']:
                exit_price = cl[i]
                exit_type = 'TW' if (exit_price - pos['entry']) * sd > 0 else 'TL'
        ...
        # ── ENTRY CHECK ──
        # Drawdown halt (only halts NEW entries; doesn't close existing)
        if APEX_DRAWDOWN_BRAKE and peak_equity > 0:
            dd = (peak_equity - equity) / peak_equity
            if dd >= APEX_DD_HALT_LIMIT:
                continue

        long_n = sum(1 for p in positions if p['dir'] == 1)
        short_n = sum(1 for p in positions if p['dir'] == -1)
        remaining = max_positions - len(positions)
        if remaining <= 0:
            continue

        candidates = trades_by_bar.get(i, [])
        used_triggers = set()
        for t_in in candidates:
            ...
            if tn in used_triggers:
                continue
            used_triggers.add(tn)
            direction = 1 if t_in['direction'] == 'LONG' else -1

            if direction == 1 and long_n >= APEX_MAX_TRADES_PER_DIR:
                continue
            if direction == -1 and short_n >= APEX_MAX_TRADES_PER_DIR:
                continue

            # ── STEP 3: Layer APEX enhancements on top of ML Kelly sizing ──
            # Base size_mult comes from smart_filter_opt (already Kelly-scaled)
            base_size_mult = t_in.get('size_mult', 1.0)
            ml_prob = t_in.get('meta_prob', 0.5)

            # Compute APEX boost/cut multiplier (on top of base)
            # v45.2: pass recent_pnls for volatility targeting
            apex_mult = _compute_apex_boost(df, i, direction, equity, peak_equity,
                                              recent_pnls=[t['pnl'] for t in trades_out])
            if apex_mult <= 0:
                continue  # drawdown halt

            final_size_mult = base_size_mult * apex_mult
            final_size_mult = float(np.clip(final_size_mult, APEX_SIZE_FLOOR, APEX_SIZE_CAP))

            # Dynamic exit params
            tp_rr_dyn, max_bars_dyn = _compute_apex_exit_params(df, i, direction)
            ...
            sl_distance = atr_val * SL_ATR
            sl_price = entry_price - sl_distance if direction == 1 else entry_price + sl_distance
            tp_price = entry_price + sl_distance * tp_rr_dyn if direction == 1 else entry_price - sl_distance * tp_rr_dyn

            # risk_amount uses the ML Kelly base × APEX boost
            risk_amount = equity * RISK_PER_TRADE * final_size_mult
            # v45.2: Hard cap — single trade can't risk > APEX_MAX_RISK_PER_TRADE
            max_risk = equity * APEX_MAX_RISK_PER_TRADE
            ...
            positions.append({
                'dir': direction, 'entry': entry_price, 'sl': sl_price, 'tp': tp_price,
                'size': position_size, 'ebar': i, 'trigger_id': tn,
                'size_mult': final_size_mult, 'max_bars': max_bars_dyn,
                'ml_prob': ml_prob,
            })
            remaining -= 1
            if direction == 1:
    ...
    for pos in positions:
        ...
        trades_out.append({
            'pnl': pnl, 'result': exit_type, 'time': df.index[-1],
            'direction': 'LONG' if sd == 1 else 'SHORT', 'actual_rr': 0,
            'bars_held': n - 1 - pos['ebar'], 'trigger_id': pos['trigger_id'],
            'ebar': pos['ebar'], 'size_mult': pos.get('size_mult', 1.0),
            'ml_prob': pos.get('ml_prob', 0.5),
            'raw_pnl_per_unit_size': (exit_price - pos['entry']) * sd,
            'entry_price': pos['entry'], 'sl': pos['sl'], 'tp': pos['tp'],
        })
...
def _compute_apex_boost(df: pd.DataFrame, i: int, direction: int,
                          equity: float, peak_equity: float,
                          recent_pnls: list = None) -> float:
    """v45.2: Compute APEX boost/cut multiplier ON TOP of the ML Kelly base.

    Adds v45.2: Volatility Targeting — scales size based on recent realized vol
    so portfolio volatility stays near APEX_TARGET_ANNUAL_VOL.

    Returns a multiplier in [0, 1.5] (1.0 = no change to ML base).
    """
    mult = 1.0
    fb = i - 1

    # ── 1-3. Regime adjustments (using prior bar data — leak-free) ──
    if fb >= 0 and 'adx' in df.columns:
        adx_val = df['adx'].values[fb] if not np.isnan(df['adx'].values[fb]) else 20.0
        atrp = df['atrp'].values[fb] if 'atrp' in df.columns and not np.isnan(df['atrp'].values[fb]) else 0.0
        atrp_med = df['atrp'].rolling(168, min_periods=50).median().values[fb] if 'atrp' in df.columns else atrp
    ...
    # ── 4. Drawdown brake (reduces risk_amount, doesn't filter trades) ──
    if APEX_DRAWDOWN_BRAKE and peak_equity > 0:
        dd = (peak_equity - equity) / peak_equity
        if dd >= APEX_DD_HALT_LIMIT:
            return 0.0
    ...
    # ── 5. Signal quality boost ──
    if APEX_SIGNAL_QUALITY and 'signal_quality' in df.columns and fb >= 0:
        sq = df['signal_quality'].values[fb]
        if not np.isnan(sq) and sq >= APEX_QUALITY_BOOST_THRESHOLD:
            mult *= 1.20

    # ── 6. v45.2 NEW: Volatility Targeting ──
    # Scale size inversely with recent realized volatility.
    # If recent vol > target → cut size; if < target → boost size.
    if APEX_VOL_TARGETING and recent_pnls is not None and len(recent_pnls) >= 20:
        recent = np.array(recent_pnls[-APEX_VOL_LOOKBACK:])
        # Convert PnL to returns (relative to current equity)
        if equity > 0:
            recent_returns = recent / equity
            realized_vol = recent_returns.std()
            if realized_vol > 0:
                # Annualize: ~365 trades/year on H1 (1 trade/day avg)
                # But our trades are sparse, so use sqrt(n_trades/year)
                trades_per_year = max(len(recent_pnls) / max(1, (i / 24 / 365)), 1)
                ann_vol = realized_vol * np.sqrt(trades_per_year)
                # Vol scalar: target / realized
                vol_scalar = APEX_TARGET_ANNUAL_VOL / ann_vol if ann_vol > 0 else 1.0
                vol_scalar = float(np.clip(vol_scalar, APEX_VOL_FLOOR, APEX_VOL_CAP))
                mult *= vol_scalar

    # ── 7. v45.5 NEW: Low-vol bonus (was a hard trigger-level gate; now just
    #      a small size boost so it no longer kills freq in high-vol weeks) ──
    if APEX_LV_BONUS and fb >= 0 and 'lv' in df.columns:
        lv_val = df['lv'].values[fb]
        if not (isinstance(lv_val, float) and np.isnan(lv_val)) and bool(lv_val):
            mult *= APEX_LV_BONUS_MULT
...
def main():
    ...
    print(f"        WR:     {base_wr:.1f}% → {s_best['wr']:.1f}%  (Δ={s_best['wr']-base_wr:+.1f}%, "
          f"{((s_best['wr']/base_wr-1)*100 if base_wr > 0 else 0):+.1f}%)")   # PATCH: guard div-by-zero (print only)
    ...
    if WF_ENABLED:
        ...
        print(f"     WR:     {wf_base_wr:.1f}% → {s_wfbest['wr']:.1f}%  "
              f"(Δ={s_wfbest['wr']-wf_base_wr:+.1f}%, {((s_wfbest['wr']/wf_base_wr-1)*100 if wf_base_wr > 0 else 0):+.1f}%)")   # PATCH: guard div-by-zero (print only)
