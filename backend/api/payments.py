...
async def _process_nowpayments_ipn(data: dict):
    ...
    async with async_session_factory() as db:
        try:
            ...
            await db.commit()
            logger.info(f"Crypto payment auto-confirmed for user {user.telegram_id}")

        except Exception as e:
            await db.rollback()
            logger.exception(
                f"NOWPayments IPN processing failed for order {order_id}: {e}"
            )
            # ── Alert the admin ──────────────────────────────────────
            # This used to be a bare `print()` that vanished into stdout
            # with no rollback visibility to anyone. A failure here means
            # a user PAID and did NOT get activated — that's a critical,
            # customer-facing bug, not a log line to lose in a scroll
            # buffer. Use a fresh session since `db` above was just
            # rolled back and may be in a bad state for further writes.
            try:
                async with async_session_factory() as notif_db:
                    notif = AdminNotification(
                        notification_type=NotificationType.PAYMENT_FAILED,
                        title="NOWPayments IPN processing failed",
                        message=(
                            f"Order: {order_id}\nUser ID: {user_id}\n"
                            f"Error: {e}\n\n"
                            f"This user's crypto payment was received by NOWPayments "
                            f"but activation failed — they likely need manual review."
                        ),
                        severity="critical",
                        related_user_id=user_id if user_id else None,
                    )
                    notif_db.add(notif)
                    await notif_db.commit()
            except Exception:
                logger.exception(
                    f"Also failed to record the AdminNotification for order {order_id}"
                )
