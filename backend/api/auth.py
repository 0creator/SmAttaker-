...
from backend.utils.permissions import Permission, role_has_permission
...
def require_permission(permission: "Permission"):
    """
    Dependency factory for fine-grained admin endpoints — use this
    instead of `require_admin` on anything tier-sensitive (payments,
    settings, admin management). `require_admin` only proves "this is
    some admin"; this additionally proves the admin's tier actually
    grants the specific capability being used.

    Usage: `_admin: User = Depends(require_permission(Permission.APPROVE_PAYMENTS))`
    """
    async def _dependency(user: User = Depends(get_current_user_dep)) -> User:
        if user.role != UserRole.ADMIN:
            raise HTTPException(status_code=403, detail="Admin privileges required.")
        if not role_has_permission(getattr(user, "admin_role", None), permission):
            raise HTTPException(
                status_code=403,
                detail=f"Your admin tier does not include the '{permission.value}' permission.",
            )
        return user
    return _dependency
