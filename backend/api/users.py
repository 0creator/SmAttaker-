...
async def list_users(
    ...
    for u in users:
        try:
            ...
            # ⚠️ v45 DEFENSIVE GUARD: never let one bad user row 500 the
            # entire /api/users endpoint. Log it, build a minimal dict
            # manually from safe attribute reads, and keep going. This is
            # exactly what was happening with the old UserOut schema
            # (NULL paper_balance on legacy users) — the whole admin
            # panel was bricked by a single column.
            import logging as _lg
            _lg.getLogger("smattaker.users").warning(
                f"UserOut.model_validate failed for user {getattr(u, 'id', '?')}: {ve}. "
                f"Falling back to manual dict."
            )
            base = {
                ...
                "approved_by_admin": bool(getattr(u, "approved_by_admin", False)),
                "default_account_type": u.default_account_type or "paper",
                "paper_balance": float(getattr(u, "paper_balance", None) or 10000.0),
                "paper_initial_balance": float(getattr(u, "paper_initial_balance", None) or 10000.0),
...
async def update_user_status(
    ...
    # ⚠️ FIX: this used to build the response with a bare
    # UserOut.model_validate(user) — the exact same call that list_users
    # already had to wrap in a try/except (see the comment there: a
    # legacy user with e.g. NULL paper_balance raises a Pydantic
    # ValidationError). The list route got the defensive fallback; this
    # route never did. Effect: the status update itself succeeded and
    # committed, but the response serialization then 500'd for any
    # affected user — so clicking Activate/Ban/Deactivate looked like it
    # "did nothing" (or worse, silently applied the change while showing
    # an error) for exactly those legacy accounts. Same fallback here.
    try:
        out = UserOut.model_validate(user)
    except Exception as ve:
        import logging as _lg
        _lg.getLogger("smattaker.users").warning(
            f"UserOut.model_validate failed after status update for user {user.id}: {ve}. "
            f"Falling back to manual dict (the status change itself already committed)."
        )
        # Build an actual UserOut instance from safe pre-coerced values —
        # a raw dict here would just hit the SAME ValidationError again
        # when FastAPI's response_model re-validates the return value.
        out = UserOut(
            id=str(user.id),
            ...
            approved_by_admin=bool(getattr(user, "approved_by_admin", False)),
            default_account_type=user.default_account_type or "paper",
            paper_balance=float(getattr(user, "paper_balance", None) or 10000.0),
            paper_initial_balance=float(getattr(user, "paper_initial_balance", None) or 10000.0),
            onboarding_completed=bool(getattr(user, "onboarding_completed", False)),
            created_at=user.created_at,
        )
    return APIResponse(data=out, message=f"User status updated to {status}.")
